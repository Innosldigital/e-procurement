...
//   // More robust role extraction - check multiple possible locations
//   const metadata = user?.publicMetadata as any;
//   const unsafeMetadata = user?.unsafeMetadata as any;

//   const userRole = metadata?.role || unsafeMetadata?.role || "";
//   const normalizedRole = String(userRole)
//     .toLowerCase()
//     .trim()
//     .replace(/[\s_-]/g, "");

//   // Debug: Log the role and full metadata
//   useEffect(() => {
//     if (isLoaded && user) {
...
//     // Suppliers can ONLY see five items
...
//         "/invoices",
//       ];
//       return supplierAllowedPaths.includes(n.href);
//     }
...
const getNavItems = (role: string) => [
  ...
  { href: "/purchase-orders", icon: ShoppingCart, label: "Purchase Orders" },
  {
    href: "/suppliers",
    icon: Building2,
    label: role === "supplier" ? "Supplier Profile" : "Suppliers",
  },
  { href: "/invoices", icon: Receipt, label: "Invoices" },
  { href: "/reports", icon: BarChart3, label: "Reports" },
  { href: "/admin", icon: Settings, label: "Admin" },
...
export function Navigation({
  ...
  // Get navigation items with role-specific labels
  ...
  const items = navItems.filter((n) => {
    ...
    // If NOT supplier → HIDE /supplier-dashboard
    if (normalizedRole !== "supplier") {
