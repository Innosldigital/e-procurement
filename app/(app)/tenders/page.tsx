...
function TendersPage() {
  ...
  const loadTenders = useCallback(
    async (showLoader = true) => {
      if (showLoader) setLoading(true);
      try {
        console.log("🔄 [TendersPage] Loading tenders...");
        const result = await getTenders();

        console.log("🔍 [TendersPage] getTenders result:", {
          success: result.success,
          count: result.data?.length || 0,
          error: result.error,
        });
        ...
        if (result.success && result.data) {
          console.log("✅ [TendersPage] Setting tenders:", result.data.length);

          // Server already filtered - use data directly
          setTenders(result.data);
          ...
          console.error(
            "❌ [TendersPage] Failed to load tenders:",
            result.error
          );
          setTenders([]);
        }
  ...
  const loadStats = useCallback(async () => {
    try {
      ...
      console.error("❌ [TendersPage] Error loading stats:", error);
    }
  }, []);
  ...
  // Fetch owner names for timeline
  useEffect(() => {
    const fetchOwnerNames = async () => {
      try {
        const ids = [
        ...
        console.error("❌ [TendersPage] Error fetching owner names:", error);
      }
    };
    fetchOwnerNames();
  }, [selectedTender]);
      ...
      <div className="max-w-[1600px] mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4 lg:gap-6">
          ...
          <div className="md:col-span-2 bg-card border border-border rounded-lg order-1">
            ...
            <div className="divide-y divide-border max-h-[50vh] md:max-h-[600px] overflow-y-auto">
              {loading ? (
                ...
                <div className="p-8 text-center">
                  <p className="text-muted-foreground text-sm mb-4">
                    {isSupplier
                      ? "No tenders available matching your product categories. Check back later or update your supplier profile."
                      : "No tenders found."}
                  </p>
                  {canCreateTender && (
                    <Button size="sm" onClick={() => setShowCreate(true)}>
            ...
            <div className="p-3 border-t border-border text-xs text-muted-foreground">
              {isSupplier
                ? "Tip: Only tenders matching your registered product categories are shown"
