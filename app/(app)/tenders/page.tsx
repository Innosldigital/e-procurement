...
function TendersPage() {
  ...
  const loadTenders = useCallback(
    async (showLoader = true) => {
      ...
      try {
        ...
        if (result.success && result.data) {
          ...
          // Filter tenders for suppliers - only show tenders in their categories
          if (isSupplier && supplierCategories.length > 0) {
            filteredData = result.data.filter((tender: any) => {
              const tenderCategory = tender.category;
              if (!tenderCategory) return false;
              return supplierCategories.includes(tenderCategory);
            });
          }
          ...
          if (selectedTender) {
            const updatedSelected = filteredData.find(
              (t: any) => t._id === selectedTender._id
            );
            if (updatedSelected) {
              setSelectedTender(updatedSelected);
            } else if (filteredData.length > 0) {
              setSelectedTender(filteredData[0]);
            } else {
    ...
    [selectedTender, isSupplier, supplierCategories]
  );
  ...
  useEffect(() => {
    ...
    setCanCreateTender(["admin", "company", "superadmin"].includes(normalized));
    const supplierRole = normalized === "supplier";
  ...
  return (
    <div className="p-4 sm:p-6 min-h-screen">
      <div className="max-w-[1600px] mx-auto">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 gap-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              {isSupplier
                ? "View and bid on tenders in your product categories"
        ...
        <div className="bg-card border border-border rounded-lg p-4 mb-6">
          <div className="flex flex-wrap gap-4 sm:gap-6 text-sm">
            ...
            {isSupplier && supplierCategories.length > 0 && (
              <div>
                <span className="text-muted-foreground">Your categories:</span>{" "}
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4 lg:gap-6">
          ...
          <div className="md:col-span-2 bg-card border border-border rounded-lg order-1">
            ...
            <div className="divide-y divide-border max-h-[50vh] md:max-h-[600px] overflow-y-auto">
              {loading ? (
                ...
                <div className="p-8 text-center">
                  <p className="text-muted-foreground text-sm mb-4">
                    {isSupplier
                      ? "No tenders found in your categories."
                      : "No tenders found."}
                  </p>
                  {canCreateTender && (
                    <Button size="sm" onClick={() => setShowCreate(true)}>
            ...
            <div className="p-3 border-t border-border text-xs text-muted-foreground">
              {isSupplier
                ? "Only tenders in your registered categories are shown"
                  ...
                  selectedTender.stage !== "Closed" && (
                    <div className="bg-accent/30 border border-border rounded-lg p-3 sm:p-4">
                      <div className="flex flex-col gap-3">
                        ...
                        {selectedTender.closeDate &&
                          new Date(selectedTender.closeDate).getTime() > Date.now() && (
                            <div className="flex justify-end">
                              <Button
                                size="sm"
      ...
      {showSubmitBidForm && selectedTender && (
        <SubmitBidForm tender={selectedTender} onClose={handleCloseSubmitBidForm} />
      )}
    </div>
  );
}
