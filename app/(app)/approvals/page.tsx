...
export default async function ApprovalsPage(props: any) {
  ...
  // Calculate pending IDs from actual data
  const pendingIds = approvals
    .filter((a: any) => {
      const s = String(a.status || "").toLowerCase();
      return s.includes("awaiting") || s.includes("pending review");
  ...
  const slaBreachedCount = approvals.filter((a: any) =>
    String(a.status || "")
      .toLowerCase()
      .includes("sla breached")
  ).length;

  const highPriorityCount = approvals.filter(
    (a: any) =>
      String(a.priority || "").toLowerCase() === "high" ||
      String(a.priority || "").toLowerCase() === "critical"
  ).length;
      ...
      <main className="p-4 md:p-6">
        ...
        <Card className="mb-6">
          <CardContent className="pt-4 md:pt-6">
              ...
              {highPriorityCount > 0 && (
                <div className="flex items-center gap-2">
                  ...
                  <Badge variant="destructive" className="text-xs">
                    {highPriorityCount} High/Critical
                  </Badge>
                </div>
              )}
              {slaBreachedCount > 0 && (
            ...
            <div className="flex flex-wrap items-center gap-2 md:gap-4 text-xs text-muted-foreground">
              <span>{approvals.length} items in your approval queue</span>
              <span>{pendingIds.length} awaiting your action</span>
              {slaBreachedCount > 0 && (
                <span className="text-destructive font-medium">
                  {slaBreachedCount} breaching SLA
                </span>
              )}
            </div>
          </CardContent>
