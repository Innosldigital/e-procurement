    ...
    <div className="flex min-h-screen flex-col">
        ...
        <main className="p-4 md:p-6">
          <div className="mb-6 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div>
              <h1 className="text-xl md:text-2xl font-semibold mb-1">Approvals</h1>
            ...
            <div className="flex gap-2 md:gap-3">
              <Button asChild variant="outline" size="sm" className="flex-1 md:flex-none">
                <Link href="/admin?section=approval-workflows">Approval rules</Link>
              </Button>
              <form
              ...
              >
                <input type="hidden" name="ids" value={JSON.stringify(pendingIds)} />
                <Button size="sm" className="flex-1 md:flex-none" type="submit" disabled={pendingIds.length === 0}>
                  Bulk approve
                </Button>
              </form>
            </div>
                ...
                <div className="space-y-1">
                  {approvals.length === 0 ? (
                    ...
                    approvals.slice(0, 10).map((item: any) => (
                      <Link key={String(item._id)} href={`/approvals?id=${String(item._id)}`} className="block">
                        <div className={`p-3 rounded-lg cursor-pointer border transition-colors ${String(selected?._id) === String(item._id) ? "bg-accent/50 border-border" : "hover:bg-accent border-transparent hover:border-border"}`}>
                ...
                <div className="space-y-4 md:space-y-6">
                  ...
                  <div className="pt-4 border-t">
                    ...
                    <div className="space-y-2 text-xs">
                      <div className="flex items-start gap-2">
                        ...
                        <div>
                          <div className="font-medium">Submitted by David Kim</div>
                          <div className="text-muted-foreground">2025-03-13 • 09:32 • APAC Marketing</div>
                        </div>
                      </div>
                      <div className="flex items-start gap-2">
                        <div className="w-1.5 h-1.5 rounded-full bg-primary mt-1.5"></div>
                        <div>
                          <div className="font-medium">Auto-assigned to marketing workflow</div>
                          <div className="text-muted-foreground">2025-03-13 • 09:33 • Rule: Marketing &gt; 50K</div>
                        </div>
                      </div>
                      <div className="flex items-start gap-2">
                        <div className="w-1.5 h-1.5 rounded-full bg-warning mt-1.5"></div>
                        <div>
                          <div className="font-medium">Waiting for your decision</div>
                          <div className="text-muted-foreground">Current step • SLA: 2 business days</div>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="pt-4 border-t">
                    ...
                    <div className="flex flex-col sm:flex-row gap-2">
                      <Button variant="outline" size="sm" className="flex-1 sm:flex-none">Request changes</Button>
