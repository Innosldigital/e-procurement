...
const EditUser = ({
  ...
  if (!canEditUser()) {
    return (
      <Sheet open={isOpen} onOpenChange={onClose}>
        <SheetContent
          side="right"
          className="w-full max-w-full sm:max-w-md md:max-w-lg lg:max-w-xl xl:max-w-2xl px-4 sm:px-6 py-4"
        >
          <SheetHeader className="space-y-3">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 flex items-center justify-center rounded-lg bg-destructive/10">
                <User className="w-5 h-5 text-destructive" />
              </div>
              <div>
  ...
  return (
    <Sheet open={isOpen} onOpenChange={onClose}>
      <SheetContent
        side="right"
        className="w-full max-w-full sm:max-w-md md:max-w-lg lg:max-w-xl xl:max-w-2xl px-4 sm:px-6 py-4 flex flex-col"
        onPointerDownOutside={(e) => e.preventDefault()}
      >
        <SheetHeader className="space-y-3 pb-4">
          <div className="flex items-center gap-3">
            <Avatar className="w-9 h-9 sm:w-11 sm:h-11 ring-2 ring-offset-2 ring-primary/10">
              <AvatarImage
                src={photoUrl || userData?.photo || userData?.imageUrl}
              />
              <AvatarFallback>
