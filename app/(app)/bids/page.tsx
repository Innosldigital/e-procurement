...
export default async function BidsPage({
  ...
  try {
    ...
    // 🔥 FIX: Use the _key from the server response directly
    rows = data.map((r) => {
      const key = r._key; // Use the key from getBidsWithDetails
      detailMap[key] = r;
      return {
        _key: key,
  ...
  // 🔥 FIX: Properly decode and find the selected bid
  ...
  return (
    <div className="flex min-h-screen flex-col">
      <main className="p-4 md:p-6">
        ...
        {/* 🔥 FIX: Always render modal, let it handle open state */}
        <BidDetailsModal
